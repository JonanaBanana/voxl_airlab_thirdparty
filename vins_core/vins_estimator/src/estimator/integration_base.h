...
class IntegrationBase
{
  public:
    ...
    IntegrationBase(const Eigen::Vector3d &_acc_0, const Eigen::Vector3d &_gyr_0,
                    const Eigen::Vector3d &_linearized_ba, const Eigen::Vector3d &_linearized_bg, const imu_info &_imu_info)
...

    {
        ...
        noise.block<3, 3>(0, 0) =  (_imu_info.acc_n_ * _imu_info.acc_n_) * Eigen::Matrix3d::Identity();
        noise.block<3, 3>(3, 3) =  (_imu_info.gyr_n_ * _imu_info.gyr_n_) * Eigen::Matrix3d::Identity();
        noise.block<3, 3>(6, 6) =  (_imu_info.acc_n_ * _imu_info.acc_n_) * Eigen::Matrix3d::Identity();
        noise.block<3, 3>(9, 9) =  (_imu_info.gyr_n_ * _imu_info.gyr_n_) * Eigen::Matrix3d::Identity();
        noise.block<3, 3>(12, 12) =  (_imu_info.acc_w_ * _imu_info.acc_w_) * Eigen::Matrix3d::Identity();
        noise.block<3, 3>(15, 15) =  (_imu_info.gyr_w_ * _imu_info.gyr_w_) * Eigen::Matrix3d::Identity();

    }

    void reset(const Eigen::Vector3d &_linearized_ba, const Eigen::Vector3d &_linearized_bg){
        sum_dt = 0.0;
